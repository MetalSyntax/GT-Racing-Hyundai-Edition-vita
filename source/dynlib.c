...
static struct hostent *gethostbyname_soloader(const char *name) {
    if (name && (strstr(name, "gameloft.com") || strstr(name, "facebook") || strstr(name, "tapjoy"))) {
        return NULL;
    }
...
/*
 * Pantalla blanca al iniciar carrera (006.log: el hilo principal deja de
 * llegar a gl_swap tras "Menu Cleaning not done", sin crash dump = cuelgue).
 * Los servidores Gameloft llevan años caídos; un connect() bloqueante contra
 * un host muerto tarda minutos en fallar (timeout TCP) con la pantalla
 * congelada en el último frame del menú (fondo blanco). El juego YA sabe
 * seguir cuando la red falla rápido (tracking bSuccess=false en 006.log),
 * así que se fuerza el fallo rápido: connect no-bloqueante + select con
 * timeout de 8s. Si fcntl no funciona en este socket, se vuelve al connect
 * bloqueante original (comportamiento actual, sin regresión).
 */
#define CONNECT_TIMEOUT_SEC 8

static int connect_soloader(int sockfd, const struct sockaddr *addr, socklen_t addrlen) {
    trace_call("connect");
    ...
    if (rc == 0) {
        l_warn("connect(fd=%d): sin respuesta en %ds, fallo rápido (servidores caídos)", sockfd, CONNECT_TIMEOUT_SEC);
        errno = ETIMEDOUT;
        return -1;
    }
    if (rc < 0) {
...
so_default_dynlib default_dynlib[] = {
        ...
        { "getuid", (uintptr_t)&ret0 }, // single-user: siempre root-ish
