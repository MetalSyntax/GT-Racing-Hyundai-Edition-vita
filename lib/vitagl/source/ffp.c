...
// [port GTRHE] Robustez ante fallos de vitaShaRK en el FFP. Upstream copia desde el
// puntero devuelto sin comprobarlo y crashea (data abort en memcpy con src=NULL,
// dump 1790258054). Aquí: se vuelca fuente+log a ux0:data/shader_cache/ffp_fail_*.txt,
// se avisa al port (hook débil) y se reintenta; si todo falla se reutiliza un shader
// ya compilado del caché RAM (render incorrecto en ese draw, pero sin crash).
extern char *vgl_take_shark_log(void);
void vgl_ffp_compile_failed(const char *kind, uint64_t mask, const char *log) __attribute__((weak));

static void ffp_report_failure(const char *kind, uint64_t mask, const char *src) {
	char *log = vgl_take_shark_log();
	char fname[256];
	sprintf(fname, "ux0:data/shader_cache/ffp_fail_%s_%016llX.txt", kind, mask);
	SceUID f = sceIoOpen(fname, SCE_O_WRONLY | SCE_O_CREAT | SCE_O_TRUNC, 0777);
	if (f >= 0) {
		sceIoWrite(f, src, strlen(src));
		if (log) {
			sceIoWrite(f, "\n/* ---- vitaShaRK log ----\n", 29);
			sceIoWrite(f, log, strlen(log));
			sceIoWrite(f, "\n*/\n", 4);
		}
		sceIoClose(f);
	}
	if (vgl_ffp_compile_failed)
		vgl_ffp_compile_failed(kind, mask, log);
	if (log)
		vgl_free(log);
}

static SceGxmProgram *ffp_compile_checked(const char *src, uint32_t *size, shark_type type, const char *kind, uint64_t mask) {
	uint32_t in_size = *size;
	SceGxmProgram *t = shark_compile_shader_extended(src, size, type, compiler_opts, compiler_fastmath, compiler_fastprecision, compiler_fastint);
	if (!t) {
		ffp_report_failure(kind, mask, src);
		shark_clear_output();
		// Puede ser memoria del compilador agotada: reiniciarlo libera todo lo que retiene.
		shark_end();
		...
		*size = in_size;
		t = shark_compile_shader_extended(src, size, type, compiler_opts, compiler_fastmath, compiler_fastprecision, compiler_fastint);
	}
	char *log = vgl_take_shark_log();
	if (log)
		vgl_free(log);
...
uint8_t reload_ffp_shaders(SceGxmVertexAttribute *attrs, SceGxmVertexStream *streams, SceGxmIndexSource index_type) {
	...
	if (ffp_dirty_vert) {
		...
		if (f >= 0) {
			...
			SceGxmProgram *t = ffp_compile_checked(vshader, &size, SHARK_VERTEX_SHADER, "v", vert_shader_mask);
			if (!t && vert_shader_cache_size > 0) {
				// Último recurso: reutilizar el vertex shader más reciente
				ffp_vertex_program = vert_shader_cache[vert_shader_cache_idx].prog;
				ffp_vertex_program_id = vert_shader_cache[vert_shader_cache_idx].id;
				ffp_vertex_unif_buf_size = vert_shader_cache[vert_shader_cache_idx].unif_buf_size;
				ffp_vertex_unif_buf = vert_shader_cache[vert_shader_cache_idx].unif_buf;
				ffp_vertex_params = vert_shader_cache[vert_shader_cache_idx].vert_unifs;
				ffp_vertex_attribs = vert_shader_cache[vert_shader_cache_idx].attributes;
				ffp_dirty_vert_attr = 0xFFFF;
				ffp_dirty_vert = GL_FALSE;
				shark_clear_output();
				goto ffp_vert_done;
...
ffp_vert_done:
	...
	if (ffp_dirty_frag) {
		...
		if (f >= 0) {
			...
			SceGxmProgram *t = ffp_compile_checked(fshader, &size, SHARK_FRAGMENT_SHADER, "f", frag_shader_mask);
			if (!t) {
				// Reintento sin combiner: todas las pasadas como GL_MODULATE (misma plantilla,
				// mismos uniforms). Visualmente aproximado, pero dibuja.
#ifdef HAVE_HIGH_FFP_TEXUNITS
				sprintf(fshader, ffp_frag_src, modulate_src, alpha_op,
					mask.num_textures, mask.has_colors, mask.fog_mode, MODULATE, MODULATE, MODULATE,
					mask.lights_num, mask.shading_mode, mask.point_sprite, mask.fast_perspective_correction, mask.srgb_mode);
#else
				sprintf(fshader, ffp_frag_src, modulate_src, alpha_op,
					mask.num_textures, mask.has_colors, mask.fog_mode, MODULATE, MODULATE,
					mask.lights_num, mask.shading_mode, mask.point_sprite, mask.fast_perspective_correction, mask.srgb_mode);
#endif
				size = strlen(fshader);
				t = ffp_compile_checked(fshader, &size, SHARK_FRAGMENT_SHADER, "fmod", frag_shader_mask);
			}
			if (!t && frag_shader_cache_size > 0) {
				// Último recurso: reutilizar el fragment shader más reciente
				ffp_fragment_program = frag_shader_cache[frag_shader_cache_idx].prog;
				ffp_fragment_program_id = frag_shader_cache[frag_shader_cache_idx].id;
				ffp_fragment_unif_buf_size = frag_shader_cache[frag_shader_cache_idx].unif_buf_size;
				ffp_fragment_unif_buf = frag_shader_cache[frag_shader_cache_idx].unif_buf;
				ffp_fragment_params = frag_shader_cache[frag_shader_cache_idx].frag_unifs;
				dirty_frag_unifs = 0xFFFFFFFF;
				ffp_dirty_frag = GL_FALSE;
...
// [port GTRHE] glTexEnviv / glGetTexEnvfv: GT Racing (LibEffects::saveTUStates /
// restoreTUSetup) guarda el estado de las unidades de textura con glGetTexEnviv +
// glGetTexEnvfv y lo restaura con glTexEnviv + glTexEnvfv tras cada pasada de efectos.
// Sin estas dos, el combiner del efecto quedaba aplicado a todo lo que venía después.
void glTexEnviv(GLenum target, GLenum pname, const GLint *params) {
	switch (pname) {
	case GL_TEXTURE_ENV_COLOR: {
		// GLES: enteros mapeados linealmente de [-2^31, 2^31-1] a [-1, 1]
		GLfloat c[4];
		for (int i = 0; i < 4; i++)
			c[i] = (GLfloat)params[i] / 2147483647.0f;
	...
	case GL_RGB_SCALE:
	case GL_ALPHA_SCALE: {
		// Vía glTexEnvfv para que marque el uniform de escala como sucio
		GLfloat f = (GLfloat)params[0];
...
void glGetTexEnvfv(GLenum target, GLenum pname, GLfloat *params) {
	...
	switch (pname) {
	case GL_TEXTURE_ENV_COLOR:
		vgl_fast_memcpy(params, &tex_unit->env_color.r, sizeof(GLfloat) * 4);
		break;
#ifndef DISABLE_TEXTURE_COMBINER
	case GL_RGB_SCALE:
		*params = tex_unit->rgb_scale;
