...
const char* GetStringUTFChars(JNIEnv* env, jstring string, jboolean* isCopy) {
    ...
    if (!str->utf16 || jstr_utf16_to_utf8(str) == JNI_FALSE || !str->utf8 || !str->utf8->array) {
        fjni_logv_err("[JNI] GetStringUTFChars(env, %p, *isCopy): utf16 conversion failed", string);
        return strdup("");
    }

    char * ret = malloc(str->utf8->len + 1);
    if (ret == NULL) {
    ...
    fjni_logv_dbg("[JNI] GetStringUTFChars(env, \"%s\", *isCopy)", ret);
