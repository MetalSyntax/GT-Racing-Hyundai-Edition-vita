...
void vglSetShaderAssociationPath(const char *path) {
	// Asphalt-5-Vita: unlike vglSetShaderCachePath right above, upstream never
	// guards this call -- shark_set_shader_association_path is a Razor/devkit
	// profiler symbol not present in a release toolchain, so building without
	// HAVE_RAZOR/HAVE_DEVKIT (our case) fails to link. We never call this
	// function ourselves; guard it the same way HAVE_SHADER_CACHE guards the
	// function above so an unused feature doesn't require a devkit library.
#ifdef HAVE_RAZOR
