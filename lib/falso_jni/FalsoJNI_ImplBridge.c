...
jobject getObjectFieldValueById(jfieldID id) {
    getFieldValueById(jobject, FIELD_TYPE_OBJECT, FieldsObject, fieldsObject, fieldsObject_size, id, NULL);
}
...
jboolean jstr_utf16_to_utf8(JavaString * jstr) {
    if (!jstr || !jstr->utf16 || !jstr->utf16->array) return JNI_FALSE;
    ...
    if (!jstr->utf8->array) return JNI_FALSE;
...
jboolean jstr_utf8_to_utf16(JavaString * jstr) {
    if (!jstr || !jstr->utf8 || !jstr->utf8->array) return JNI_FALSE;

    if (jstr->utf16 == NULL) {
        jstr->utf16 = jda_alloc(jstr->utf8->len, FIELD_TYPE_CHAR);
        if (jstr->utf16 == NULL) return JNI_FALSE;
    } else if (jstr->utf16->len + 1 < jstr->utf8->len) {
        if (jda_realloc(jstr->utf16, jstr->utf8->len - 1) == JNI_FALSE) {
            return JNI_FALSE;
        }
    }

    if (!jstr->utf16->array) return JNI_FALSE;
