...
bool str_starts_with(const char * str, const char * prefix) {    char cp;
    char cs;
...
char * str_sha1sum(const char * str, size_t size) {    if (size == 0) {
        size = strlen(str);
    }
...
void hy_rewrite_path(const char * path, char * buf, unsigned int bufsz) {
    static const struct { const char * from; const char * to; } map[] = {
        { "/sdcard/Android/data/com.gameloft.android.ANMP.GloftGTHY/files/", DATA_PATH },
        { "/sdcard/Android/obb/com.gameloft.android.ANMP.GloftGTHY/", DATA_PATH "com.gameloft.android.ANMP.GloftGTHY/" },
        { "/data/data/com.gameloft.android.ANMP.GloftGTHY", DATA_PATH "saves" },
        { "/sdcard/", DATA_PATH },
    };
    ...
    for (unsigned i = 0; i < sizeof(map) / sizeof(map[0]); i++) {
        if (str_starts_with(path, map[i].from)) {
            snprintf(buf, bufsz, "%s%s", map[i].to, path + strlen(map[i].from));
            return;
        }
    }
