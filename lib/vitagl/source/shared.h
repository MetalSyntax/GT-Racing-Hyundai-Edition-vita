...
/*
 * Asphalt-5-Vita compat shim (not upstream vitaGL): the vitasdk installed in
 * this project's build environment predates vita-headers adding these named
 * constants to psp2/gxm.h (SceGxmInitializeParams::flags/SceGxmTextureFormat
 * here are otherwise complete -- just missing these specific names). Values
 * copied verbatim from vitasdk/vita-headers (github.com/vitasdk/vita-headers,
 * include/psp2/gxm.h) so the rest of vitaGL compiles and sets/compares the
 * exact same bits it would against a current vitasdk. #ifndef-guarded so
 * this becomes a no-op the moment the installed headers define these
 * themselves (e.g. after a vitasdk update) -- see also gxm.c/textures.c.
 */
...
#define SCE_GXM_TEXTURE_FORMAT_ETC1_1BGR 0x84000000u
#endif
...
#define FFP_SHADER_CACHE_MAGIC 29 // [port GTRHE] 28->29: UNPURE_TEXCOORDS cambia las máscaras; invalida shaders mal generados. This must be increased whenever ffp shader sources or shader mask/combiner mask changes
